...
echo "        curl -X POST --data-urlencode 'payload={\"username\":\"podyn\",\"channel\":\"developers\",\"icon_url\":\"\",\"attachments\":[{\"color\":\"#4183c4\",\"text\":\"Launching podyn ${AWS_REGION}\",\"footer\":\"\"}]}' https://hooks.slack.com/services/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" >> ./launch.sh
echo "    else" >> ./launch.sh
echo "        echo Restarting..." >> ./launch.sh
echo "    fi" >> ./launch.sh
echo "    echo \"starting podyn.jar ${@} &>> /app/podyn.log\"" >> ./launch.sh
echo "    exec java -jar /app/podyn.jar --postgres-jdbc-url \"${POSTGRES_JDBC_URL}\" $DYNAMODB_OPTS ${@} &>> /app/podyn.log &" >> ./launch.sh
echo "fi" >> ./launch.sh
